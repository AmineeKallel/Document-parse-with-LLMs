        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if not isinstance(text, str):
                    logging.error(
                        f"API response content is not a string for {file_name} (pages {start_index+1} to {start_index+len(images)}): {type(text)}")
                    return [""] * len(images), 0.0
                text = text.strip()
                os.makedirs(DEBUG_TEXT_DIR, exist_ok=True)
                debug_response_file = os.path.join(
                    DEBUG_TEXT_DIR, f"{Path(file_name).stem}_pages_{start_index+1}_to_{start_index+len(images)}_raw_response.txt")
                ...
                for i, t in zip(valid_images, texts):
                    result_texts[i] = t
                    debug_text_file = os.path.join(
                        DEBUG_TEXT_DIR, f"{Path(file_name).stem}_page_{start_index+i+1}_raw.txt")
                ...
                logging.info(
                    f"gpt-4.1-mini API Cost for {file_name} (pages {start_index+1} to {start_index+len(images)}):")
                ...
                logging.error(
                    f"OpenAI API error for {file_name} (pages {start_index+1} to {start_index+len(images)}, attempt {attempt+1}/{MAX_RETRIES}): {str(e)}")
                if attempt < MAX_RETRIES - 1:
                    delay = RETRY_DELAY * (2 ** attempt)
                    logging.info(f"Retrying after {delay} seconds...")
                    time.sleep(delay)
                continue
        logging.error(
            f"Failed to extract text from {file_name} (pages {start_index+1} to {start_index+len(images)}) after {MAX_RETRIES} attempts")
        return [""] * len(images), 0.0
    except Exception as e:
        logging.error(
            f"Error extracting text from {file_name} (pages {start_index+1} to {start_index+len(images)}): {str(e)}")
        return [""] * len(images), 0.0
    ...
    try:
        ...
        for chunk_idx, text_chunk in enumerate(text_chunks):
            ...
            if not isinstance(content, str):
                logging.error(
                    f"API response content is not a string for {pdf_path} chunk {chunk_idx + 1}: {type(content)}")
                continue

            content = content.strip()

            os.makedirs(DEBUG_TEXT_DIR, exist_ok=True)
            debug_raw_response_file = os.path.join(
                DEBUG_TEXT_DIR, f"{Path(pdf_path).stem}_chunk_{chunk_idx + 1}_raw_response.txt")
                ...
                try:
                    ...
                    if last_valid_index != -1:
                        ...
                        if isinstance(extracted_data, list):
                            ...
                            all_extracted_data.extend(extracted_data)
                            logging.info(
                                f"Recovered {len(extracted_data)} items from partial JSON for {pdf_path} chunk {chunk_idx + 1}")
                        else:
...
def extract_text_from_pdf(pdf_path):
    ...
    try:
        ...
        logging.info(
            f"Total API Cost for {pdf_path}: ${total_cost:.6f} (Text Extraction: ${total_text_cost:.6f}, Structured Extraction: ${struct_cost:.6f})")
