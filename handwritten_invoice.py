...
def image_to_base64(image, file_name="unknown", image_index=0):
    ...
    try:
        ...
        logging.error(f"Error converting image to base64 for {file_name} (image {image_index}): {str(e)}", exc_info=True)
        return None
...
def extract_text_from_image(image, image_index=0, file_name="unknown"):
    ...
    try:
        ...
        logging.debug(f"Calling OpenAI API (model: gpt-4.1-mini) for text extraction from {file_name} (image {image_index})")
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                logging.error(f"OpenAI API error for {file_name} (image {image_index}, attempt {attempt+1}/{MAX_RETRIES}): {str(e)}", exc_info=True)
...
def process_all_invoices(invoice_texts, output_excel=None):
    ...
    try:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                logging.debug(f"Calling OpenAI API (model: gpt-4) for field extraction (attempt {attempt+1}/{MAX_RETRIES})")
